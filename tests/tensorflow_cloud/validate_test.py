...
class TestValidate(unittest.TestCase):
    ...
    def test_valid_args(self):
        validate.validate(
            ...
            pip_libraries=['tensorflow', 'tensorflow-gpu'],
        ...
        validate.validate(
            ...
            requirements_txt='tests/testdata/requirements.txt',
            pip_libraries=['tensorflow', 'tensorflow-gpu', 'absl'],
            chief_config=TestValidate.VALID_MACHINE_CONFIG,
            worker_config=None,
            worker_count=0,
            region='us-central1',
    ...
    def test_invalid_pip_libraries(self):
        with pytest.raises(ValueError, match=r'Invalid `pip_libraries`'):
            validate.validate(
                entry_point='tests/testdata/mnist_example_using_fit.py',
                distribution_strategy='auto',
                requirements_txt='tests/testdata/requirements.txt',
                pip_libraries='tensorflow==1.15.2, absl',
                chief_config=TestValidate.VALID_MACHINE_CONFIG,
                worker_config=TestValidate.VALID_MACHINE_CONFIG,
                worker_count=1,
                region='us-central1',
