...
class GoogleApiClientTest(tf.test.TestCase):
  ...
  def setUp(self):
    ...
    self.mock_apiclient.projects().jobs().get.return_value = self.mock_request
    self.mock_apiclient.projects().jobs(
    ).cancel.return_value = self.mock_request
    self._local_config_path = os.path.join(self.get_temp_dir(), "config.json")
  ...
  def test_wait_for_aip_training_job_completion_non_blocking_success(self):
    ...
    self.mock_apiclient.projects().jobs().get.assert_called_with(name=job_name)
  ...
  def test_get_client_environment_name_with_kaggle(self):
    ...
    self.assertEqual(google_api_client.get_client_environment_name(),
  ...
  @mock.patch.object(
      google_api_client, "get_or_set_consent_status", autospec=True)
  def test_TFCloudHttpRequest_with_rejected_consent(self, mock_consent_status):
    ...
    self.assertDictEqual({"user-agent": f"tf-cloud/{version.__version__} ()"},
                         http_request.headers)

  @mock.patch.object(
      google_api_client, "get_or_set_consent_status", autospec=True)
  @mock.patch.object(
      google_api_client, "get_client_environment_name", autospec=True)
  def test_TFCloudHttpRequest_with_consent(self, mock_get_env_name,
                                           mock_consent_status):
  ...
  @mock.patch.object(
      google_api_client, "get_client_environment_name", autospec=True)
  def test_TFCloudHttpRequest_with_additional_metrics(self, mock_get_env_name,
                                                      mock_consent_status):
  ...
  @mock.patch.object(google_api_client, "_is_module_present", autospec=True)
  @mock.patch.object(google_api_client, "_get_env_variable", autospec=True)
  def test_is_explainable_ai_sdk_installed_false(self, mock_getenv,
                                                 mock_modules):
    mock_getenv.return_value = None
    mock_modules.return_value = False
    self.assertEqual(google_api_client.is_explainable_ai_sdk_installed(), False)

  @mock.patch.object(google_api_client, "_is_module_present", autospec=True)
  @mock.patch.object(google_api_client, "_get_env_variable", autospec=True)
  def test_is_explainable_ai_sdk_installed_true(self, mock_getenv,
                                                mock_modules):
    mock_getenv.return_value = None
    mock_modules.return_value = True
    self.assertEqual(google_api_client.is_explainable_ai_sdk_installed(), True)
